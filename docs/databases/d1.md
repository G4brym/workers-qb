...
D1 supports a maximum of [100 bound parameters per query](https://developers.cloudflare.com/d1/platform/limits/). `workers-qb` assigns each independently supplied bound value a unique parameter position across CTEs, subqueries, `WHERE`, and `HAVING` clauses. As a result, a query composed from several nested clauses may consume more parameter positions than an equivalent query that deliberately reuses one numbered placeholder.

Split queries or use D1 batch operations when a query approaches this limit. Do not use `Raw` to bypass the limit for request-controlled values; `Raw` inserts SQL directly and must only contain trusted, application-authored expressions.
